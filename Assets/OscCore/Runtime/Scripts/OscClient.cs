...
namespace OscCore
{
    public class OscClient
    {
        ...
        //ADDED BY THE EGG
        public void SendPosQuart(string address, string stringName, Vector3 position, Quaternion quart)
        {
            m_Writer.Reset();
            m_Writer.Write(address);
            const string typeTags = ",sfffffff";
            m_Writer.Write(typeTags);
            m_Writer.Write(stringName);
            m_Writer.Write(position);
            m_Writer.Write(quart);
            m_Socket.Send(m_Writer.Buffer, m_Writer.Length, SocketFlags.None);
        }
    }
}